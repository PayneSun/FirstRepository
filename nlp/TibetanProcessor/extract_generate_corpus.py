...
def delete_filename_in_data(data_with_filename):
    """
        删除文件中的文件名
    """
    with open(data_with_filename, 'r', encoding="utf-16-le") as fin, \
            open(data_with_filename + '.pure', 'w', encoding="utf-16-le") as fout:
        for line in fin:
            if '.txt' not in line:
                print(line)
                fout.write(line)
...
if __name__ == "__main__":
    ...
    # 生成语料
    # get_ner_in_patch("E:\\tibetan-ner", 'tb_dataset.ld')
    # get_ner_in_patch(".\\data\\fwg-tibetan", 'fwg_tibetan.txt.ld')

    get_ner_in_patch(".\\data\\fwg-data2", 'fwg2.txt.ld')

    # 剔除文件名
    # delete_filename_in_data('tb_dataset.ld')
